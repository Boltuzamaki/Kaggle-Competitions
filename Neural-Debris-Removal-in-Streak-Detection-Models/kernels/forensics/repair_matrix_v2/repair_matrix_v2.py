...
# %% [markdown]
# # Neural Debris - positive-retention repair matrix v2
#
# One maximal GPU run:
#
# - six bias-frozen classification-head repair candidates,
...
OUT = Path("/kaggle/working/repair_matrix_v2")
...
CANDIDATES = [
    {"name": "score_w_pos16", "scope": "score_weight", "lr": 3e-5, "positive": 16.0, "negative": 0.25, "anchor": 1e-4},
    {"name": "score_w_pos32", "scope": "score_weight", "lr": 3e-5, "positive": 32.0, "negative": 0.25, "anchor": 1e-4},
    {"name": "tail_w_pos16", "scope": "tail_weights", "lr": 2e-5, "positive": 16.0, "negative": 0.25, "anchor": 3e-4},
    {"name": "tail_w_pos32", "scope": "tail_weights", "lr": 2e-5, "positive": 32.0, "negative": 0.25, "anchor": 3e-4},
    {"name": "full_w_pos32", "scope": "full_weights", "lr": 1e-5, "positive": 32.0, "negative": 0.25, "anchor": 5e-4},
    {"name": "full_w_pos64", "scope": "full_weights", "lr": 1e-5, "positive": 64.0, "negative": 0.25, "anchor": 5e-4},
]
CV_STEPS = 20
...
figure.tight_layout()
figure.savefig(OUT / "repair_matrix_v2.png", dpi=180)
plt.close(figure)
...
(OUT / "repair_report_v2.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
(Path("/kaggle/working") / "REPAIR_MATRIX_V2_COMPLETE.txt").write_text(
    f"Repair matrix v2 complete. Selected {selected_name}.\n",
    encoding="utf-8",
)
log("ALL DONE", selected=selected_name, report=str(OUT / "repair_report_v2.json"))
