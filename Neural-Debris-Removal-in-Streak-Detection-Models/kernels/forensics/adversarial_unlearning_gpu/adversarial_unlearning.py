...
# %% [markdown]
# # Neural Debris - detector-aware adversarial unlearning
#
# One maximal GPU run:
#
# - six native RetinaNet fine-tuning candidates,
# - five source-grouped validation folds,
# - targeted negative focal loss on poison-associated P3/P4 anchors,
# - poison boxes treated as background,
# - non-poison public-unlearn detections retained as positive boxes,
...
OUT = Path("/kaggle/working/adversarial_unlearning")
...
CANDIDATES = [
    {"name": "cls_lr1e5", "scope": "full_cls", "lr": 1e-5, "anchor": 5e-4},
    {"name": "cls_lr3e5", "scope": "full_cls", "lr": 3e-5, "anchor": 5e-4},
    {"name": "cls_lr1e4", "scope": "full_cls", "lr": 1e-4, "anchor": 5e-4},
    {"name": "fpn_cls_lr1e5", "scope": "fpn_cls", "lr": 1e-5, "anchor": 8e-4},
    {"name": "fpn_cls_lr3e5", "scope": "fpn_cls", "lr": 3e-5, "anchor": 8e-4},
    {"name": "late_cls_lr1e5", "scope": "late_cls", "lr": 1e-5, "anchor": 1e-3},
]
CV_STEPS = 40
FINAL_STEPS = [40, 80, 160]
...
CONFIG = {
    ...
    "repair": "native RetinaNet loss with poison boxes omitted and retained public-unlearn detections as positives",
    "rule_guard": {
        "test_images": False,
...
def augment(image_id, seed, allow_transplant=True):
    ...
    clean_boxes = [box.copy() for box in clean_boxes_by_image[image_id]]

    k = int(rng.integers(0, 4))
    flip = bool(rng.integers(0, 2))
    source_image = image
    image, target_boxes[0] = d4_image_box(source_image, target_boxes[0], k, flip)
    clean_boxes = [d4_image_box(source_image, box, k, flip)[1] for box in clean_boxes]
    ...
    clean_boxes = [transform_box_affine(box, matrix) for box in clean_boxes]
    ...
    if allow_transplant and rng.random() < 0.5:
        ...
        for _ in range(50):
            ...
            occupied = target_boxes + clean_boxes
            if max(box_iou_numpy(candidate, occupied), default=0.0) < 0.01:
                ...
                break
    clean_boxes = [
        box for box in clean_boxes
        if (box[2] - box[0]) >= 2 and (box[3] - box[1]) >= 2
    ]
    return image.astype(np.float32), target_boxes, clean_boxes
...
def configure_scope(model, scope):
    ...
    for name, parameter in model.named_parameters():
        ...
        if scope == "score_weight":
            ...
            enabled = (
                "head.cls_score" in name
                or "head.cls_subnet" in name
                or name.startswith("backbone.fpn_")
            )
        elif scope == "late_cls":
            enabled = (
                "head.cls_score" in name
                or "head.cls_subnet" in name
                or name.startswith("backbone.fpn_")
                or "backbone.bottom_up.res5" in name
            )
...
clean_boxes_by_image = {}
for image_id, (prediction_boxes, prediction_scores) in teacher_predictions.items():
    target_overlap = box_iou_numpy(boxes[image_id], prediction_boxes)
    keep = (prediction_scores >= 0.20) & (target_overlap < 0.10)
    clean_boxes_by_image[image_id] = [
        box.astype(np.float32) for box in prediction_boxes[keep]
    ]
log(
    "Public-unlearn retention boxes frozen",
    total=sum(len(items) for items in clean_boxes_by_image.values()),
    per_image={str(key): len(value) for key, value in clean_boxes_by_image.items()},
)
teacher_export = {
    str(image_id): {
...
def train_model(candidate, train_ids, steps, run_name, checkpoints=()):
    ...
    history = []
    checkpoint_paths = {}
    with heartbeat(run_name), EventStorage(start_iter=0) as storage:
        for step in range(1, steps + 1):
            image_id = train_ids[(step - 1) % len(train_ids)]
            image, target_boxes, clean_boxes = augment(
                image_id,
                SEED + zlib.crc32(run_name.encode("utf-8")) % 100000 + step,
                allow_transplant=True,
            )
            training_record = to_record(image)
            instances = Instances((1024, 1024))
            clean_tensor = torch.as_tensor(
                np.asarray(clean_boxes, np.float32).reshape(-1, 4),
                dtype=torch.float32,
            )
            instances.gt_boxes = Boxes(clean_tensor)
            instances.gt_classes = torch.zeros(len(clean_tensor), dtype=torch.int64)
            training_record["instances"] = instances
            optimizer.zero_grad(set_to_none=True)
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                loss_dict = student([training_record])
                detection_loss = sum(loss_dict.values())
                drift_terms = [
                    (parameter - original_parameters[name]).pow(2).mean()
                    for name, parameter in student.named_parameters()
                    if parameter.requires_grad
                ]
                drift = torch.stack(drift_terms).mean()
                objective = detection_loss + candidate["anchor"] * drift
            ...
            row = {
                ...
                "step": step,
                "image_id": image_id,
                "poison_boxes": len(target_boxes),
                "retained_boxes": len(clean_boxes),
                "loss": float(objective.detach().cpu()),
                "loss_cls": float(loss_dict["loss_cls"].detach().cpu()),
...
figure.tight_layout()
figure.savefig(OUT / "adversarial_unlearning.png", dpi=180)
plt.close(figure)
...
(OUT / "adversarial_unlearning_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
(Path("/kaggle/working") / "ADVERSARIAL_UNLEARNING_COMPLETE.txt").write_text(
    f"Detector-aware unlearning complete. Selected {selected_name}.\n",
    encoding="utf-8",
)
log("ALL DONE", selected=selected_name, report=str(OUT / "adversarial_unlearning_report.json"))
