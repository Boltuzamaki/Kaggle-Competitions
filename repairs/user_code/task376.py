# Source: predicted/test_onnx_task376.py — ONNX graph construction code
# Verified model: repairs/task376.onnx
import json
import numpy as np
import onnx
...
def create_onnx_model():
    ...
    graph = helper.make_graph(nodes, 'task376_graph', [input_info], [output_info])
    model = helper.make_model(graph, producer_name='task376_model', opset_imports=[helper.make_opsetid('', 15)])
    onnx.save(model, 'task376.onnx')


# Build the model (the function saves it internally, so we load the result)
create_onnx_model()
import glob
model = onnx.load("/project/repairs/task376.onnx")
