# Source: predicted/test_onnx_task302.py — ONNX graph construction code
# Verified model: repairs/task302.onnx
import json
import numpy as np
import onnx
...
def create_onnx_model():
    ...
    graph = helper.make_graph(nodes, 'task302_graph', [input_info], [output_info])
    model = helper.make_model(graph, producer_name='task302_model', opset_imports=[helper.make_opsetid('', 15)])
    onnx.save(model, 'task302.onnx')


# Build the model (the function saves it internally, so we load the result)
create_onnx_model()
import glob
model = onnx.load("/project/repairs/task302.onnx")
