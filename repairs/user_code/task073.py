...
def create_model():
    ...
    inits = [
        K('c0_i64', [0]), K('c1_i64', [1]), K('cm1_i64', [-1]),
        K('c_depth_10', [10]), K('c_oh_vals', [0.0, 1.0], dtype=np.float32),
        K('ax_2_1d', [2]), K('ax_2_3_1d', [2, 3]),
    ...
    nodes.append(helper.make_node('Cast', ['is_1'], ['is_1_i64'], to=I64))
    nodes.append(helper.make_node('ReduceMax', ['is_1_i64'], ['col_has_1_i64'], axes=[2], keepdims=1))
    nodes.append(helper.make_node('Cast', ['col_has_1_i64'], ['col_has_1'], to=TensorProto.BOOL))
    
    nodes.append(helper.make_node('Greater', ['argmax', 'c0_i64'], ['gt_0']))
    nodes.append(helper.make_node('Where', ['gt_0', 'r', 'cm1_i64'], ['r_valid']))
    nodes.append(helper.make_node('ReduceMax', ['r_valid'], ['r_max'], axes=[2, 3], keepdims=1))
    
    nodes.append(helper.make_node('Equal', ['r', 'r_max'], ['is_bottom']))
    nodes.append(helper.make_node('And', ['is_bottom', 'col_has_1'], ['becomes_1']))
    
    nodes.append(helper.make_node('Where', ['is_1', 'c0_i64', 'argmax'], ['inter_val']))
    nodes.append(helper.make_node('Where', ['becomes_1', 'c1_i64', 'inter_val'], ['out_val']))
    ...
    graph = helper.make_graph(nodes, 'task073', [x], [y], inits)
