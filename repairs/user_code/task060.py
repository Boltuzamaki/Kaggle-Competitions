...
def create_model():
    ...
    inits = [
        ...
        K('c0_1d', [0], dtype=np.int64),
        K('c1_1d', [1], dtype=np.int64),
    ...
    nodes.append(helper.make_node('Greater', ['argmax', 'c0_i64'], ['has_color']))
    nodes.append(helper.make_node('Where', ['has_color', 'cols', 'c0_i64'], ['col_indices']))
    
    nodes.append(helper.make_node('ReduceMax', ['col_indices'], ['c_right'], axes=[3], keepdims=1))
    nodes.append(helper.make_node('Greater', ['c_right', 'c0_i64'], ['is_active']))
    
    nodes.append(helper.make_node('Div', ['c_right', 'c2_i64'], ['c_center']))
    
    nodes.append(helper.make_node('Less', ['cols', 'c_center'], ['mask_left_raw']))
    nodes.append(helper.make_node('And', ['is_active', 'mask_left_raw'], ['mask_left']))
    
    nodes.append(helper.make_node('Equal', ['cols', 'c_center'], ['mask_center_raw']))
    nodes.append(helper.make_node('And', ['is_active', 'mask_center_raw'], ['mask_center']))
    
    nodes.append(helper.make_node('Greater', ['cols', 'c_center'], ['gt_center']))
    nodes.append(helper.make_node('LessOrEqual', ['cols', 'c_right'], ['le_right']))
    nodes.append(helper.make_node('And', ['gt_center', 'le_right'], ['mask_right_raw']))
    nodes.append(helper.make_node('And', ['is_active', 'mask_right_raw'], ['mask_right']))
    
    nodes.append(helper.make_node('Slice', ['argmax', 'c0_1d', 'c1_1d', 'ax_3_1d'], ['color_left']))
    nodes.append(helper.make_node('GatherElements', ['argmax', 'c_right'], ['color_right'], axis=3))
    
    nodes.append(helper.make_node('Where', ['mask_left', 'color_left', 'c0_i64'], ['out_1']))
    nodes.append(helper.make_node('Where', ['mask_right', 'color_right', 'out_1'], ['out_2']))
    nodes.append(helper.make_node('Where', ['mask_center', 'c5_i64', 'out_2'], ['out_final']))
