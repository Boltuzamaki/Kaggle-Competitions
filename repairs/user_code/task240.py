# Source: predicted/test_onnx_task240.py — ONNX graph construction code
# Verified model: repairs/task240.onnx
import json
import numpy as np
import onnx
...
def create_onnx_model():
    ...
    graph = helper.make_graph(nodes, 'task240_graph', [input_info], [output_info])
    model = helper.make_model(graph, producer_name='task240_model', opset_imports=[helper.make_opsetid('', 15)])
...
create_onnx_model()
import glob
model = onnx.load("/project/repairs/task240.onnx")
