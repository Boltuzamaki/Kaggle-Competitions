# Source: predicted/test_onnx_task142.py — ONNX graph construction code
# Verified model: repairs/task142.onnx
import json
import numpy as np
import onnx
...
def create_onnx_model():
    """
    Task 142: Mirror horizontally and then mirror vertically to double size.
    """
    ...
    graph = helper.make_graph(nodes, 'task142_graph', [input_info], [output_info])
    model = helper.make_model(graph, producer_name='task142_model', opset_imports=[helper.make_opsetid('', 15)])
    onnx.save(model, 'task142.onnx')

def check_task():
    import onnxruntime as ort
    
    create_onnx_model()
    session = ort.InferenceSession('task142.onnx')
    
    with open('task142.json', 'r') as f:
        task = json.load(f)
...
create_onnx_model()
import glob
model = onnx.load("/project/repairs/task142.onnx")
