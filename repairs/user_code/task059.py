...
def create_model():
    ...
    inits = [
        ...
        K('weight', np.ones((1, 1, 3, 3), dtype=np.float32), dtype=np.float32),
        K('pads', [0, 0, 0, 0, 0, 0, 3, 3], dtype=np.int64),
    ...
    nodes.append(helper.make_node('Equal', ['argmax', 'c5_i64'], ['is_5']))
    nodes.append(helper.make_node('Equal', ['argmax', 'c0_i64'], ['is_0']))
    nodes.append(helper.make_node('Or', ['is_5', 'is_0'], ['is_5_or_0']))
    nodes.append(helper.make_node('Not', ['is_5_or_0'], ['is_color']))
    
    nodes.append(helper.make_node('Cast', ['is_color'], ['is_color_f'], to=F))
    
    nodes.append(helper.make_node('Conv', ['is_color_f', 'weight'], ['sums'], strides=[4, 4]))
    nodes.append(helper.make_node('Round', ['sums'], ['sums_round']))
    nodes.append(helper.make_node('Cast', ['sums_round'], ['sums_i64'], to=I64))
    
    nodes.append(helper.make_node('ReduceMax', ['sums_i64'], ['max_sum'], axes=[2, 3], keepdims=1))
    nodes.append(helper.make_node('Equal', ['sums_i64', 'max_sum'], ['is_max']))
    nodes.append(helper.make_node('Cast', ['is_max'], ['is_max_f'], to=F))
    
    nodes.append(helper.make_node('ConvTranspose', ['is_max_f', 'weight'], ['filled_f'], strides=[4, 4]))
    nodes.append(helper.make_node('Pad', ['filled_f', 'pads'], ['filled_padded_f']))
    
    nodes.append(helper.make_node('Greater', ['filled_padded_f', 'c0_5_f'], ['filled_mask']))
    
    nodes.append(helper.make_node('Where', ['is_color', 'argmax', 'c0_i64'], ['color_pixels']))
    nodes.append(helper.make_node('ReduceMax', ['color_pixels'], ['fill_color'], axes=[2, 3], keepdims=1))
    
    nodes.append(helper.make_node('Where', ['filled_mask', 'fill_color', 'c0_i64'], ['out_1']))
    nodes.append(helper.make_node('Where', ['is_5', 'c5_i64', 'out_1'], ['out_final']))
    ...
    graph = helper.make_graph(nodes, 'task059', [x], [y], inits)
