# Source: predicted/test_onnx_task186.py — ONNX graph construction code
# Verified model: repairs/task186.onnx
import json
import numpy as np
import onnx
...
def create_onnx_model():
    ...
    graph = helper.make_graph(nodes, 'task186_graph', [input_info], [output_info])
    model = helper.make_model(graph, producer_name='task186_model', opset_imports=[helper.make_opsetid('', 15)])
    onnx.save(model, 'task186.onnx')


# Build the model (the function saves it internally, so we load the result)
create_onnx_model()
import glob
model = onnx.load("/project/repairs/task186.onnx")
