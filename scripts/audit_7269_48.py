"""
Full 400-task audit comparing our current repairs/ against submissions/7269.48/, using the exact
scorer methodology (sanitize_model + score_network, onnx.checker full_check, all
train+test+arc-gen examples). Never trusts the folder's implied aggregate score -- only a
task-by-task real comparison decides anything, per the lesson learned from the earlier
7268-folder regression this session.

Usage: python scripts/audit_7269_48.py
Writes results to scratch_onnx/audit_7269_48_results.json
"""
import copy
import functools
import json
...
REP = "repairs"
OTHER = os.path.join("submissions", "7269.48")
OUT_FILE = os.path.join("scratch_onnx", "audit_7269_48_results.json")
...
for t in range(1, 401):
    ...
    other_r = audit_one(other_path, t, f"a726948_{t}")
    status = other_r[3]
    ...
    if status == "ok":
        ...
        results[t] = {"our_pts": round(our_pts, 4), "their_pts": round(their_pts, 4),
                      "their_status": status, "delta": delta}
        if delta > 0:
            clean_wins.append((t, our_pts, their_pts, delta))
...
print(f"\n=== DONE: {len(clean_wins)} clean genuine wins, {len(negpad_matches)} negative-pads-pattern tasks (need isolation tests to evaluate) ===")
clean_wins.sort(key=lambda x: -x[3])
for t, our_pts, their_pts, delta in clean_wins:
    print(f"  task{t:03d}: {our_pts:.4f} -> {their_pts:.4f} ({delta:+.4f})")
print(f"\nnegative-pads-pattern tasks (cost unmeasurable locally, would need isolated Kaggle tests): {negpad_matches}")
