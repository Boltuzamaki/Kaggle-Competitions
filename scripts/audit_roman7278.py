"""
Full 400-task audit comparing our current repairs/ against
neurogolf-submission-7278-onnx, using the exact scorer methodology. Never
trust the folder's implied aggregate score -- only a task-by-task real
...
REP = "repairs"
OTHER = os.path.expandvars(r"C:\Users\chand\.cache\kagglehub\datasets\romantamrazov\neurogolf-submission-7278-onnx\versions\1")
OUT_FILE = os.path.join("scratch_onnx", "audit_roman7278_results.json")
...
for t in range(1, 401):
    ...
    other_r = audit_one(other_path, t, f"aroman7278_{t}")
    status = other_r[3]
    ...
    if status == "ok":
        ...
        results[t] = {"our_pts": round(our_pts, 4), "their_pts": round(their_pts, 4),
                      "their_status": status, "delta": delta, "their_cost": other_r[1]}
        if delta > 0.0005:
