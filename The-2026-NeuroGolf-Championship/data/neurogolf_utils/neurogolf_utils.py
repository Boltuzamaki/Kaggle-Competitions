...
def calculate_memory(model, trace_path):
    ...
    for tensor_name in tensor_names:
        ...
        if not item: print(f"Missing item {tensor_name}"); return None
        if item.type.HasField("sequence_type"): print("sequence_type"); return None
        if not item.type.HasField("tensor_type"): continue
        tensor_type = item.type.tensor_type
        if not tensor_type.HasField("shape"): print(f"No shape for {tensor_name}"); return None
        num_elements = 1
        for dim in tensor_type.shape.dim:
            if dim.HasField("dim_param"): print(f"dim_param in {tensor_name}"); return None
            if not dim.HasField("dim_value"): print(f"no dim_value in {tensor_name}"); return None
            if dim.dim_value <= 0: print(f"dim_value <= 0 in {tensor_name}: {dim.dim_value}"); return None
