# Source: predicted/test_onnx_task141.py — ONNX graph construction code
# Verified model: repairs/task141.onnx
import json
import numpy as np
import onnx
...
def create_onnx_model():
    ...
    graph = helper.make_graph(nodes, 'task141_graph', [input_info], [output_info])
    model = helper.make_model(graph, producer_name='task141_model', opset_imports=[helper.make_opsetid('', 15)])
...
create_onnx_model()
import glob
model = onnx.load("/project/repairs/task141.onnx")
