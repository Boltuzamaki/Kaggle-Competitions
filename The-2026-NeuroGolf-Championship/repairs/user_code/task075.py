...
def create_model():
    ...
    inits = [
        K('c1_i64', [1]), K('c0_f', [0.0], dtype=np.float32),
        K('pads', [0, 0, 1, 1, 0, 0, 1, 1]),
        K('ax_2_3', [2, 3]), K('ax_2_1d', [2]),
    ...
    nodes.append(helper.make_node('Equal', ['argmax', 'c1_i64'], ['is_1']))
    nodes.append(helper.make_node('Cast', ['is_1'], ['is_1_f'], to=F))
    
    nodes.append(helper.make_node('Pad', ['is_1_f', 'pads', 'c0_f'], ['padded_is_1']))
    ...
    for dy in [-1, 0, 1]:
        for dx in [-1, 0, 1]:
            ...
            name_suffix = f'_{dy}_{dx}'.replace('-', 'm')
            
            starts_m = K(f'starts_m{name_suffix}', [sy, sx])
            ends_m = K(f'ends_m{name_suffix}', [sy+30, sx+30])
            mask = f'mask{name_suffix}'
            nodes.append(helper.make_node('Slice', ['padded_is_1', starts_m.name, ends_m.name, 'ax_2_3'], [mask]))
            
            starts_b = K(f'starts_b{name_suffix}', [dy+1, dx+1])
            ends_b = K(f'ends_b{name_suffix}', [dy+2, dx+2])
            bval = f'bval{name_suffix}'
            nodes.append(helper.make_node('Slice', ['input', starts_b.name, ends_b.name, 'ax_2_3'], [bval]))
            
            term = f'term{name_suffix}'
            nodes.append(helper.make_node('Mul', [mask, bval], [term]))
            terms.append(term)
            masks.append(mask)
            inits.extend([starts_m, ends_m, starts_b, ends_b])
            
    nodes.append(helper.make_node('Sum', terms, ['summed_oh']))
    nodes.append(helper.make_node('Sum', masks, ['sum_masks']))
    nodes.append(helper.make_node('Greater', ['sum_masks', 'c0_f'], ['is_any_mask']))
    
    nodes.append(helper.make_node('Where', ['is_any_mask', 'summed_oh', 'input'], ['final_oh']))
    
    nodes.append(helper.make_node('ArgMax', ['final_oh'], ['pred_argmax'], axis=1, keepdims=1))
    ...
    graph = helper.make_graph(nodes, 'task075', [x], [y], inits)
