...
def create_model():
    ...
    inits = [
        ...
        K('ax_2_1d', [2]),
        K('ax_2_3_1d', [2, 3]),
        K('conv_weight', np.ones((1, 1, 3, 3), dtype=np.float32), dtype=np.float32)
    ]
    ...
    for c in range(1, 10):
        c_i64 = f'c{c}_i64'
        if c not in [0, 1, 2]:
            inits.append(K(c_i64, [c]))
            
        nodes.append(helper.make_node('Equal', ['argmax', c_i64], [f'eq_{c}']))
        nodes.append(helper.make_node('Cast', [f'eq_{c}'], [f'eq_{c}_i64'], to=I64))
        nodes.append(helper.make_node('ReduceSum', [f'eq_{c}_i64', 'ax_2_3_1d'], [f'count_{c}'], keepdims=1))
        nodes.append(helper.make_node('Equal', [f'count_{c}', 'c1_i64'], [f'is_unique_{c}']))
        nodes.append(helper.make_node('And', [f'is_unique_{c}', f'eq_{c}'], [f'center_{c}']))
        center_masks.append(f'center_{c}')
        
    nodes.append(helper.make_node('Or', [center_masks[0], center_masks[1]], ['c_or_1']))
    for i in range(2, 9):
        nodes.append(helper.make_node('Or', [f'c_or_{i-1}', center_masks[i]], [f'c_or_{i}']))
        
    nodes.append(helper.make_node('Identity', ['c_or_8'], ['center_mask']))
    nodes.append(helper.make_node('Cast', ['center_mask'], ['center_mask_f'], to=F))
    
    nodes.append(helper.make_node('Conv', ['center_mask_f', 'conv_weight'], ['conv_out'], pads=[1,1,1,1]))
    nodes.append(helper.make_node('Greater', ['conv_out', 'c0_5_f'], ['surround_mask']))
    
    nodes.append(helper.make_node('Where', ['surround_mask', 'c2_i64', 'c0_i64'], ['surround_val']))
    nodes.append(helper.make_node('Where', ['center_mask', 'argmax', 'surround_val'], ['out_val']))
