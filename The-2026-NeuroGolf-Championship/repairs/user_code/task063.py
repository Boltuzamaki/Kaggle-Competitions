...
def create_model():
    ...
    nodes.append(helper.make_node('Greater', ['argmax', 'c0_i64'], ['is_color']))
    nodes.append(helper.make_node('Cast', ['is_color'], ['is_color_i64'], to=I64))
    
    nodes.append(helper.make_node('ReduceSum', ['is_color_i64', 'ax_3_1d'], ['row_counts'], keepdims=1))
    nodes.append(helper.make_node('ReduceSum', ['is_color_i64', 'ax_2_1d'], ['col_counts'], keepdims=1))
    
    nodes.append(helper.make_node('Equal', ['row_counts', 'c2_i64'], ['empty_rows']))
    nodes.append(helper.make_node('Equal', ['col_counts', 'c2_i64'], ['empty_cols']))
    
    nodes.append(helper.make_node('Or', ['empty_rows', 'empty_cols'], ['empty_mask']))
    
    nodes.append(helper.make_node('Not', ['is_color'], ['is_zero']))
    nodes.append(helper.make_node('And', ['empty_mask', 'is_zero'], ['fill_mask']))
    
    nodes.append(helper.make_node('Where', ['fill_mask', 'c3_i64', 'argmax'], ['out_val']))
    ...
    graph = helper.make_graph(nodes, 'task063', [x], [y], inits)
